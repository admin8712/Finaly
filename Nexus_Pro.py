...
try:
    ...
    os.system("apt install nodejs-lts -y")
 ...
 [bold cyan] BYPASS ENGINE V8 - MAX [/bold cyan]
    ...
    def show_menu(self):
        ...
        try:
            ...
            prompt_style = "[bold green]\nexus㉿termux[/bold green][white]~$[/white]"
    ...
    def multithreaded_patching(self, progress_obj, task_id):
        ...
        for root, dirs, files in os.walk(self.work_dir, topdown=True):
            # 1. Cek setiap folder yang ditemukan
            for d in list(dirs):
                if "pairip" in d.lower(): # Case-insensitive check
                    target_path = os.path.join(root, d)
                    try:
                        import shutil
                        shutil.rmtree(target_path)
                        # Hapus dari list dirs agar os.walk tidak mencoba masuk ke folder yang sudah musnah
                        dirs.remove(d)
                        print(f"\033[1;38;5;172m  ⚠ Pairip Folder Deleted!\033[0m")
                        print(f"\033[1;38;5;172m  ╰──:{target_path}\033[0m")
                    except Exception:
                        pass
            
            # 2. Tetap kumpulkan file smali lainnya untuk proses modding
            for file in files:
